...
1.  **Dynamic Power** - consumed by activity in a circuit.  Computed by:
    ```
    P = (1/2) * C * V^2 * f * α
    ```
    where:
    ...
    *   `f` = frequency
    *   `α` = activity factor (% of transistors active each clock cycle)
...
*   Speedup - "X is N times faster than Y" (X new, Y old)
    ...
    *   Speedup = latency(Y) / latency(X) =  `(CPI(Y) * CTime(Y)) / (CPI(X) * CTime(X))` (notice Y/X reversal)
...
*   More Stages → more hazards (CPI ↑), but less work per stage ( cycle time ↓)
*   5-stage pipeline: Fetch-Decode-Execute-Memory-Write

## Branch Prediction

*   Predictor must compute `PC_next` based only on knowledge of `PC_now`
    1.  Guess "is this a branch?"
    2.  Guess "is it taken?"
    3.  "and if so, what is the target PC"
*   Accuracy:  `CPI = 1 + (mispred/inst) * (penalty/mispred)`
    *   `(mispred/inst)` is determined by the predictor accuracy.
    *   `(penalty/mispred)` is determined by the pipeline depth at misprediction.
*   Types of Predictors and components
    *   Not-Taken: Always assume branch is not taken
    *   Historical: `PC_next = f(PC_now, history[PC_now])`
...
*   IPC should never assume "perfect processor", so ILP ≥ IPC.
